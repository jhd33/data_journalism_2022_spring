...
* **Analysis summary**: I first read in the csv, then changed the data type of the amount column. Then I needed to filter by date and by name. I did each one in their own dataframe to keep it as clear as possible. 
...
taxbroker_donations<-nc_finance_data_cleaned %>% 
  mutate(date = mdy(date)) %>% 
  filter(date >= '2018-01-10' & date <= '2018-07-04') %>% 
  filter(str_detect(cleaned_employer, "Monarch")) %>% 
