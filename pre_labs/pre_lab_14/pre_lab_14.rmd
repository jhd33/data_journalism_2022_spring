...
**Answer**
The dataframe shows all the tweets in the dataset, with information on who tweeted it, which state they represent, and which party they are from. 
```{r}
...
**Answer**
The unique function shows how that each of these words exist as a unique word, even though the only difference is the capitalization. 
...
**Answer**
This function makes all of the unique words into the same case. 
...
**Answer**
It shows how many times each unique word appears. It is not useful because it includes mostly prepositions. 
...
**Answer**
Yes it is more interesting because it shows the actual content, not just conjunctions and common joiners. 
```{r}
unique_words %>%
...
**Answer**
This chart shows that covid was mentioned in nearly double the percentage of tweets in 2021 vs 2020. 
```{r}
unique_words_2020 %>%
  anti_join(stop_words) %>%
  group_by(word) %>%
...
**Task** Run the following code and describe the results.
**Answer**
Unsuprisingly, the results are all related to the pandemic. 
```{r}
covid_tweets %>%
  filter(created < '2021-01-01') %>%
...
**Task** Run the following code and describe the results.
**Answer**
The biggest difference here is that vaccines become a more prominent discussion point. 
```{r}
covid_tweets %>%
  filter(created >= '2021-01-01') %>%
  unnest_tokens(bigram, content, token = "ngrams", n = 2) %>%
...
**Task** Run the following code and describe the results.
**Answer**
Terms relating to public health appeared more in the Democrat tweets than the Republican ones. 
```{r}
covid_tweets %>%
...
**Task** Run the following code and describe the results.
**Answer**
The Democrats focused more on Trump, while the Republicans focused on terms like "vaccine mandates". 
...
**Answer**
The results are counts of several words, and whether they were positive or negative. For the most part, the sentiments seem pretty intuititve. 
```{r}
