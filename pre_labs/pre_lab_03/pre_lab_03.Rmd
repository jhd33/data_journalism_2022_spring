...
**Answer**
No, there are no parsing errors
...
**Answer**:
There are 135073 loans with NA. 
...
**Answer**:
There are 135073 loans, and yes it is the same amount. 
...
**Answer**:
There are 135073 loans, and yest they are the same as the previous two tasks. 
...
**Answer**
Max amount is 10 million dollars. The minium is six dollars. The max is not suspicious because 10 million was the max amount for the PPP program. In theory, the minimum is suspicious, but when the number is so low, the question is whether or not it is newsworthy. 
```{r}
...
**Answer** The data type is character. 
```{r}
...
**Answer**
One problem is the inconsistent styling in the column names. For example NAME is capitalized while amount is not. Second, the amount is a character not a number, so the math won't work. Third, the first two rows are exactly the same. 
...
**Answer**
It adds an x before the name of the column so that it does not begin with a number. 
...
**Answer**
It says that character is an invalid type of argument. This is because the amounts are characters and not numbers, so it cannot use them for mathematical equations. 
```{r}
# cleaning function
...
**Answer**
There are two duplicate rows. The business name is Stuart Jones LLC.
