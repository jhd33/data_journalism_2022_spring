...
**Answer**
It has 63 columns and 195865 rows. 
```{r}
...
**Answer**
The group_by dictates that we are trying to find the loans per county. The summarize dictates what we are looking for, ans the arrange puts the loans in descending order. The head dictates how many results come back. 
```{r}
...
**Answer**
A blank square with nothing in it is the result. 
```{r}
maryland_ppp_top_counties %>%
  ggplot()
```
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer**
The code produces a bar chart, although a very messy one. 
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
...
**Answer**
The results have been sorted in ascending order. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
The difference from the last code block is the coord_flip. This flipped the axes and put the bars going horizontally instead of vertically. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
The theme_minimal was added which makes the layout a lot cleaner and easier to read. 
...
**Answer**
The theme_economist was added, which changes the color and design. 
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
A title was added, the axes were labeled differently, and a caption was provided. 
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
A ranked list of loans from least to greatest by month. 
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
This shows a line chart of total loans over time. 
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
The chart is the same but the grid and lines are different and are labelled differently. 
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
The layout was cleaned up after the grid was changed.
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
