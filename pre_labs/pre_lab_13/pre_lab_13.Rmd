...
**Answer**
This is a regular dataframe that shows the political voting in each state, and the PPP loan data. 
```{r}
...
**Answer**
The line shows a relatively correlative, although not that strong, relationship between Trump votes and PPP loans recieved. 
```{r}
...
**Task** Run the following code. Describe the plot. What do you see?
**Answer**
This correlation is weaker than the previous one. As the line gets more horizontal, the weaker the correlation becomes. 
```{r}
...
**Answer**
The graph below shows the correlation coefficient for the first two graphs. The numbers confirm that the correlation in the first graph was stronger. 
```{r}
reveal_data %>%
...
**Answer**
This table shows the t, and p values, along with the confidence interval. These are calculations to show how statistically valid the correlations are. 
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer**
The p value here is much higher, indicating that there might be less validity to the statistics. 
```{r}
...
**Answer**
In both metrics, Trump voting states received more money per person and business. 
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer**
The p value here is very high, showing that we would likely not be able to use this statistic for anything meaningful. 
```{r}
...
**Task** Run the following code. Describe the output. What do you see?
**Answer**
This is another t-test. This time, the p-value is very small, which indicates that the data is statistically menaingful. Although it is still imoportant to not confuse correlation with causation. 
```{r}
