...
**Answer**
There are two columns, one which shows the naics sector and the other that matches that sector to a specific code. 
...
**Answer**
I see the html code for the entire webpage. 
...
**Answer**
The output has converted the html code into data that R recognizes. The output is then grouped by the tags within the html code. 
```{r}
# read in the html
...
**Answer**
Under the naics_industry, there are three groups nested under the <html> tag. They are <head>, <body>, and (xml attributes)
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**
You see the page source html but you see where the selected element starts. In other words, you can see exactly where the html code for the table exists within the document. 
...
**Answer** 
The output created dataframes of all the tables within the document. 
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**
There are six dataframes nested under naics_industry and that page shows the numbers of rows and columns that exist within each one. 
...
**Answer**
The scroll button reveals dataframes with all the rows and columns, in other words it reveals the table we scraped. 
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**
This is the original table (2b) that we wanted to scrape from the web. 
...
**Answer**
naics_industry is now just the dataframe we are looking for. 
...
**Answer**
The output looks the same, but it now only has 20 rows. I cannot tell how we cleaned the names, as they appear to be the same. 
...
**Answer**
The output is the cleaned dataframe from the end of part 1. 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
The web page has a number of seperate tables for different types of information regarding sector 22. 
...
**Task** Run the following code to read in the html. Briefly describe the output that appears below the codeblock.
**Answer**
We have the nested data again under employment_info. 
...
**Answer**
The data is still nested under a few specific tags.
...
**Answer**
Now the data exists in a proper dataframe. 
...
**Answer**
Only one row ecist in th dataframe, with two columns. 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
The page looks exactly the same as the last one, just for a different sector. 
...
**Answer**
The words from each line are displayed below. 
```{r}
...
**Answer**
The outcome looks exactly the same as task 14. 
```{r}
...
**Answer**
The orginal error was that list_of_sectors was called something different. It has been fixed and the output now resembles the previous tasks. 
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer**
There is the following error message: Error in print(sector_name) : object 'sector_name' not found. 
...
**Answer**
The code made a list of numbers 1-10. 
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
for (number in 1:10) {
  print(number)
...
**Answer**
The outpit is the cleaned dataframe from part 1. 
...
**Answer**
It just has the numbe rof rows (20). 
```{r}
nrow(naics_industry)
```
...
**Answer**
There us a list of numbers that goes up to 20, the number of rows in the dataframe. 
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
...
**Answer**
The output just shows the number of employees for November 2021, which was 536.9. 
```{r}
...
**Task** Run the following code to load table from last chapter. Briefly describe the output that appears below the codeblock.
**Answer**
The output is the original cleaned dataframe from last chapter. 
```{r}
# Define url of page we want to scrape
...
**Answer**
The output is the cleaned dataframe with an added column that includes the sector url. 
...
**Answer**
The code created a new column called sector_xpath_id that was built from the urls. 
```{r}
...
**Answer**
The output is the same as the previous task without the Public Administration row. 
...
**Answer**
The output is 19 seperate dataframes. Each dataframe only has 1 row. 
...
**Answer**
The output is a list of row numbers and the url that corresponds to them. 
```{r}
...
**Answer**
The output is 19 seperate nested lists. 
```{r}
...
**Answer**
The output is actual dataframes that only jave one row and has clean names. 
...
**Answer**
The output is the same, except the dataframes now have each_row_df added back. 
...
**Task** Run the following for loop to clean up tables. Briefly describe the output that appears below the codeblock.
**Answer**
The dataframes are now cleaned. 
```{r}
...
**Answer**
The output is the final cleaned dataframe, all in one dataframe with all of the sectors added. 
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
The table of workforce statistics is one of many tables on the webpage. 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
The agriculture page only has unemployment rate and not the other metrics.  
...
**Answer**
The dataframe now has only 19 rows. 
